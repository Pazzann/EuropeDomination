	...
	public override void ToGUIHandleEvent(ToGUIEvent @event)
	{
		switch (@event)
		{
			case ToGUIUpdateCountryInfo:
				...
				_moneyLabel.Text = EngineState.MapInfo.Scenario.Countries[EngineState.PlayerCountryId].Money
					.ToString("N0");
				_manpowerLabel.Text = EngineState.MapInfo.Scenario.Countries[EngineState.PlayerCountryId].Manpower
					.ToString();
				return;
			default:
				return;
