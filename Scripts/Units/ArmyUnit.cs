...
public partial class ArmyUnit : Node2D
{
	...
	{
		var spriteSize = _armyUnitSprite.SpriteFrames
			.GetFrameTexture(_armyUnitSprite.Animation, _armyUnitSprite.Frame).GetSize();
		var thisRect = new Rect2(GlobalPosition - spriteSize * Scale / 2, spriteSize * Scale);
		return rect.Intersects(thisRect);
	}

	public static void SelectUnits(Array<Node> allUnits, List<ArmyUnit> selectedUnits)
	{
		foreach (var armyUnit in allUnits) ((ArmyUnit)armyUnit).IsSelected = false;

		foreach (var armyUnit in selectedUnits) armyUnit.IsSelected = true;
	}
}
