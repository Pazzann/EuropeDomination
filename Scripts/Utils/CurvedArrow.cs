...
public partial class CurvedArrow : Node2D
{
    ...
    public void Setup(List<Vector2> points)
    {
        ...
        {
            var p = points[i].ProjectOntoLine(points[i - 1], points[i + 1]);
    ...
    public void DrawLine()
    {
        ...
        _line2D.DefaultColor = Color.Color8(255, 0, 0);
        _line2D.Width = 100;
        _line2D.JointMode = Line2D.LineJointMode.Round;
        foreach (var point in _path2D.Curve.GetBakedPoints()) _line2D.AddPoint(point);
    }
    ...
    private int IndexOfClosestBakedPoint(Vector2 point)
    {
        ...
        var res = -1;
        var bakedPoints = _path2D.Curve.GetBakedPoints();
    ...
    private void DrawProgressLine()
    {
        ...
        _progressLine2D.DefaultColor = Color.Color8(0, 255, 0);
        ...
        var bakedPoints = _path2D.Curve.GetBakedPoints();
        var index = IndexOfClosestBakedPoint(_pathFollow2D.Position);
        for (var i = 0; i <= index; i++) _progressLine2D.AddPoint(bakedPoints[i]);
