...
public partial class ArmyHandler : GameHandler
{
    ...
    public override void Init()
    {
        ...
        foreach (var data in countryData.Value.Units)
            ...
            {
                var obj = _armyScene.Instantiate() as ArmyUnit;
                obj.SetupUnit(a, EngineState.MapInfo, this);


                //TODO: NORMAL CALCULATION OF ARMY POSITION
                obj.Position = new Vector2(EngineState.MapInfo.Scenario.Map[data.CurrentProvince].CenterOfWeight.X + 5,
                    EngineState.MapInfo.Scenario.Map[data.CurrentProvince].CenterOfWeight.Y);
                AddChild(obj);
            }
    }
    ...
    private void _addArmyUnit(ArmyUnitData a)
    {
        ...
        //TODO: NORMAL CALCULATION OF ARMY POSITION
        obj.Position = new Vector2(EngineState.MapInfo.Scenario.Map[a.CurrentProvince].CenterOfWeight.X + 5,
            EngineState.MapInfo.Scenario.Map[a.CurrentProvince].CenterOfWeight.Y);
        AddChild(obj);
    }
        ...
        if (@event is InputEventMouseButton { ButtonIndex: MouseButton.Right, Pressed: true })
        {
            ...
            if (!_isShiftPressed)
                foreach (var armyUnit in EngineState.MapInfo.CurrentSelectedUnits)
                {
                    ...
                    armyUnit.NewPath(
                        PathFinder.FindPathFromAToB(armyUnit.Data.CurrentProvince, tileId,
                            EngineState.MapInfo.MapProvinces(ProvinceTypes.LandProvinces)));
                ...
                foreach (var armyUnit in EngineState.MapInfo.CurrentSelectedUnits)
                {
                    ...
                    {
                        armyUnit.NewPath(PathFinder.FindPathFromAToB(armyUnit.Data.CurrentProvince, tileId,
                            EngineState.MapInfo.MapProvinces(ProvinceTypes.LandProvinces)));
                        return false;
                    }

                    armyUnit.AddPath(
                        PathFinder.FindPathFromAToB(armyUnit.Data.MovementQueue[0].Key, tileId,
                            EngineState.MapInfo.MapProvinces(ProvinceTypes.LandProvinces)));
                }
    ...
    public override void ViewModUpdate(float zoom)
    {
        if (EngineState.MapInfo.CurrentMapMode == MapTypes.Political && zoom > 3.0f)
        {
            ...
            foreach (ArmyUnit unit in GetChildren()) unit.Visible = vision[unit.Data.CurrentProvince];
            return;
        }
    ...
    public override void GUIInteractionHandler(GUIEvent @event)
    {
        switch (@event)
        {
            ...
            case GUIMergeUnitsEvent e:
                ...
                if (e.UnitsToMerge.Count(d => d.Data.CurrentProvince == unit.Data.CurrentProvince) !=
                    e.UnitsToMerge.Count)
                    return;
                for (var i = 1; i < e.UnitsToMerge.Count; i++)
                {
    ...
    {
        foreach (var armyUnit in GetChildren()) (armyUnit as ArmyUnit).UpdateDayTick();
    }
