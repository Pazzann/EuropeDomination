...
public partial class MapHandler : GameHandler
{
    ...
    public override bool InputHandle(InputEvent @event, int tileId)
    {
            ...
            EngineState.MapInfo.MapColors[tileId] != MapDefaultColors.Unselectable)
        {
            ...
            {
                var route = new TransportationRoute(tileId, EngineState.MapInfo.CurrentSelectedProvinceId,
                    _goodToTransport,
        ...
        if (EngineState.MapInfo.Scenario.Map[EngineState.MapInfo.CurrentSelectedProvinceId] is LandProvinceData data)
            switch (@event)
            {
                case GUIBuildBuildingEvent e:
                    var province =
                        (LandProvinceData)EngineState.MapInfo.Scenario.Map[
    ...
    private void _drawArrow(TransportationRoute route)
    {
        ...
        arrow.GlobalPosition = Math.MathUtils.VectorCenter(map[route.ProvinceIdFrom].CenterOfWeight,
            map[route.ProvinceIdTo].CenterOfWeight);

        float arrowScale =
            (map[route.ProvinceIdTo].CenterOfWeight - map[route.ProvinceIdFrom].CenterOfWeight).Length() /
    ...
    public override void DayTick()
    {
        ...
        if (EngineState.MapInfo.CurrentSelectedProvinceId > -1 &&
            EngineState.MapInfo.Scenario.Map[
                EngineState.MapInfo.CurrentSelectedProvinceId] is LandProvinceData landData)
        {
    ...
    public override void MonthTick()
    {
        ...
        foreach (LandProvinceData data in EngineState.MapInfo.Scenario.Map.Where(data => data is LandProvinceData))
        {
            ...
            if (data.HarvestedTransport != null)
            {
                var diff = data.Resources[(int)data.Good] -
                           Mathf.Max(data.Resources[(int)data.Good] - data.HarvestedTransport.Amount, 0);
                data.Resources[(int)data.Good] -= diff;
                (EngineState.MapInfo.Scenario.Map[data.HarvestedTransport.ProvinceIdTo] as LandProvinceData).Resources[
                    (int)data.HarvestedTransport.TransportationGood] += diff;
            }
        }
            ...
            foreach (SpecialBuilding building in data.SpecialBuildings.Where(b => b != null))
            {
                if (building is Factory factory && factory.Recipe != null)
                {
                    if (factory.Recipe.Ingredients.Where(ingredient =>
                                data.Resources[(int)ingredient.Key] - ingredient.Value * factory.ProductionRate < 0)
                            .ToArray().Length <= 0)
                    {
                    ...
                    {
                        var diff = data.Resources[(int)factory.Recipe.Output] - Mathf.Max(
                            data.Resources[(int)factory.Recipe.Output] - factory.TransportationRoute.Amount, 0);
                        data.Resources[(int)factory.Recipe.Output] -= diff;
                        (EngineState.MapInfo.Scenario.Map[factory.TransportationRoute.ProvinceIdTo] as LandProvinceData)
                            .Resources[(int)factory.Recipe.Output] += diff;
                    }
                }
                ...
                if (building is StockAndTrade stockAndTrade)
                {
                    foreach (var route in stockAndTrade.TransportationRoutes)
                    {
                        if (route != null)
                        {
                            var diff = data.Resources[(int)route.TransportationGood] -
                                       Mathf.Max(data.Resources[(int)route.TransportationGood] - route.Amount, 0);
                            data.Resources[(int)route.TransportationGood] -= diff;
                            (EngineState.MapInfo.Scenario.Map[route.ProvinceIdTo] as LandProvinceData).Resources[
                                (int)route.TransportationGood] += diff;
                        }
                    }
                }
            }
